...
exports['findup'] = {
  ...
  'simple': function(test) {
    test.expect(8);
    var opts = {cwd: 'spec/fixtures/a/b'};
    test.equal(rel(findup('foo.txt', opts)), path.normalize('spec/fixtures/a/foo.txt'), 'should find files');
    test.equal(rel(findup('bar.txt', opts)), path.normalize('spec/fixtures/a/b/bar.txt'), 'should find files');
    test.equal(rel(findup('a.txt', opts)), path.normalize('spec/fixtures/a.txt'), 'should find files');
    test.equal(rel(findup('?.txt', opts)), path.normalize('spec/fixtures/a.txt'), 'should support glob patterns');
    test.equal(rel(findup('*.txt', opts)), path.normalize('spec/fixtures/a/b/bar.txt'), 'should find the first thing that matches the glob pattern');
    test.equal(rel(findup(['b*.txt', 'f*.txt'], opts)), path.normalize('spec/fixtures/a/b/bar.txt'), 'should find the first thing that matches any of the glob patterns');
    test.equal(rel(findup(['f*.txt', 'b*.txt'], opts)), path.normalize('spec/fixtures/a/b/bar.txt'), 'should find the first thing that matches any of the glob patterns');
    test.equal(findup('not-gonna-exist-i-hope.txt', opts), null, 'should returning null if no files found');
    test.done();
  },
  'cwd': function(test) {
    test.expect(8);
    process.chdir('spec/fixtures/a/b');
