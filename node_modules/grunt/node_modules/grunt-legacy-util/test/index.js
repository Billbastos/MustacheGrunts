...
exports['util.spawn'] = {
  setUp: function(done) {
    this.script = path.resolve('spec/fixtures/spawn.js');
    done();
  },
  ...
  'cmd not in path': function(test) {
    ...
    util.spawn({
      cmd: 'spec\\fixtures\\exec' + (win32 ? '.cmd' : '.sh'),
    }, function(err, result, code) {
      test.equals(err, null);
      test.equals(code, 0);
  ...
  'grunt': function(test) {
    ...
    util.spawn({
      grunt: true,
      args: [ '--gruntfile', 'spec/fixtures/Gruntfile-print-text.js', 'print:foo' ],
    }, function(err, result, code) {
      test.equals(err, null);
      test.equals(code, 0);
  ...
  'grunt passes execArgv': function(test) {
    ...
    util.spawn({
      cmd: process.execPath,
      args: [ '--harmony', process.argv[1], '--gruntfile', 'spec/fixtures/Gruntfile-execArgv.js'],
    }, function(err, result, code) {
      test.equals(err, null);
      test.equals(code, 0);
...
exports['util.spawn.multibyte'] = {
  setUp: function(done) {
    this.script = path.resolve('spec/fixtures/spawn-multibyte.js');
    done();
  },
