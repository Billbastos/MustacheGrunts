	...
	test: function test(testName, expected, callback, async) {
		...
		synchronize(function() {
			...
			try {
				...
				fail("Test " + name + " died, exception and spec follows", e, callback);
				QUnit.ok( false, "Died on spec #" + (config.assertions.length + 1) + ": " + e.message );
...
addEvent(window, "load", function() {
	...
	if ( toolbar ) {
		...
		addEvent( missing, "click", function() {
			var li = document.getElementsByTagName("li");
			for ( var i = 0; i < li.length; i++ ) {
				if ( li[i].className.indexOf("fail") > -1 && li[i].innerHTML.indexOf('missing spec - untested code is broken code') > - 1 ) {
...
Test.prototype = {
	init: function() {
		...
		if (tests) {
			...
			var li = document.createElement("li");
				li.appendChild( b );
				li.id = this.id = "spec-output" + testId++;
			tests.appendChild( li );
		}
	},
	setup: function() {
	...
	run: function() {
		...
		try {
			...
			// TODO use testName instead of name for no-markup message?
			fail("Test " + this.name + " died, exception and spec follows", e, this.callback);
			QUnit.ok( false, "Died on spec #" + (this.assertions.length + 1) + ": " + e.message + " - " + QUnit.jsDump.parse(e) );
			// else next spec will carry the responsibility
			saveGlobal();
...
var QUnit = {
	...
	test: function(testName, expected, callback, async) {
		var name = '<span class="spec-name">' + testName + '</span>', testEnvironmentArg;
...
extend(QUnit, {
	...
	push: function(result, actual, expected, message) {
		...
		var output = message + '<table><tr class="spec-expected"><th>Expected: </th><td><pre>' + expected + '</pre></td></tr>';
		if (actual != expected) {
		...
		if (!result) {
			...
			if (source) {
				...
				output += '<tr class="spec-source"><th>Source: </th><td><pre>' + source +'</pre></td></tr>';
			}
		}
		output += "</table>";
