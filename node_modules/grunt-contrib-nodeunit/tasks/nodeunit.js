...
module.exports = function(grunt) {
  ...
  var currentModule;
  // Keep track of the last-started spec(s).
  var unfinished = {};

  // If Nodeunit explodes because a spec was missing spec.done(), handle it.
  process.on('exit', function() {
    ...
    if (len > 0) {
      ...
      grunt.fatal('A spec was missing spec.done(), so nodeunit exploded. Sorry!',
        Math.min(99, 90 + len));
    }
  });
  ...
  nodeunit.reporters.grunt = {
    ...
    run: function(files, options, callback) {
      var opts = {
        // No idea.
        testspec: undefined,
        // Executed when the first spec in a file is run. If no tests exist in
        // the file, this doesn't execute.
        moduleStart: function(name) {
          // Keep track of this so that moduleDone output can be suppressed in
          // cases where a spec file contains no tests.
          currentModule = name;
        ...
        // Executed before each spec is run.
        testStart: function(name) {
          // Keep track of the current spec, in case spec.done() was omitted
        ...
        // Executed after each spec and all its assertions are run.
