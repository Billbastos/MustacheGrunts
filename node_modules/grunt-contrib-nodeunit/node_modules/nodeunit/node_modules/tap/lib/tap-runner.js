...
function Runner (options, cb) {
  ...
  // Temporary spec files bunkerified we'll remove later
  this.f2delete = []
...
Runner.prototype.run = function() {
  ...
  function finish (er) {
    ...
    // Cleanup temporary spec files with coverage:
...
// glob the filenames so that spec/*.js works on windows
...
function setOptionsForDebug(self) {
  ...
  // 30 seconds is not a long time to debug your spec.
...
// Prepend to every spec file to run. Note tap.spec at the very top due it
// "plays" with include paths.
Runner.prototype.coverHeader = function() {
  // semi here since we're injecting it before the first line,
  // and don't want to mess up line numbers in the spec files.
  return "var ___TAP_COVERAGE = require("
       + JSON.stringify(require.resolve("runforcover"))
       + ").cover(/.*/g);"
}

// Append at the end of every spec file to run. Actually, the stuff which gets
