...
// Every "spec" is also a harness.  If they do not have a harness,
...
// - "skip" in the spec config obj should skip it.
...
Harness.prototype.process = function () {
  ...
  // to the next spec if the current one is actually over.
...
Harness.prototype.end = function () {
  ...
  if (this._plan !== null) {
    ...
    if (total !== this._plan) {
      this.results.add(assert.equal(total, this._plan, "spec count != plan"))
    }
    this._plan = total
  }
...
Harness.prototype.childEnd = function (child) {
  ...
  this.results.add(child.conf.name || "(unnamed spec)")
...
Harness.prototype.test = function test (name, conf, cb) {
  ...
  //console.error("making spec", [name, conf, cb])
