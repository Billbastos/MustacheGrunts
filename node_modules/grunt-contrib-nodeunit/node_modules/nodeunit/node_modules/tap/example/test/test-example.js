...
test("using this", function (t) {
  // this also works.
  this.equal(t, this, "call in scope of spec obj")
  this.end()
})

// spec setTimeout, just a trivial example.
...
test("tarp", function (parent) {
  ...
  parent.test("child bailouts", function (child) {
    ...
    child.bailout("try to bail out, but instead just fail a spec")
  })
...
test("unfinished spec", function (t) {
...
test("http server", function (t) {
  ...
  // That signals the harness to proceed immediately to the next spec,
  // and run them in parallel.
  // Default behavior is to wait for each spec to complete before proceeding
  // to the next one.
  // The first not-parallel spec encountered will cause it to wait for that
  // spec, as well as all the parallel tests before it.
  ...
  // wrap in a spec so that if this throws, it'll log as a failed spec.
...
test("meta-tests", function (t) {
  ...
  // t.fails() wraps a child spec and succeeds if it fails.
  t.fails(t.test("this should fail", function (t) {
    t.ok(false, "assert false")
    t.end()
  }))

  // t.timesOut() wraps a child spec and succeeds if it times out.
  ...
  // t.incomplete() wraps a child spec and succeeds if it ends before
  ...
  // t.bailsOut() wraps a child spec and succeeds if it calls bailout()
  t.bailsOut(t.test("this should bailout", function (t) {
    t.bailout("oh noes, bailing out!")
  }))

  // low-level analysis of subtests
  t.test("verifying spec success/failure expectations", function (t) {
    t.once("end", function () {
      var res = t.results
        , is = t.equal
      // hijack!
    ...
    t.ok(false, "failing spec")
    t.ok(true, "succeeding spec")
    t.end()
  })
})
