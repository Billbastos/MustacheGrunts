...
test("Parse Variables", function() {
  var text = "spec{{foo}}spec!{{bar}}spec!!{{baz}}spec!!!";
  var tree = Hogan.parse(Hogan.scan(text));
  is(tree.length, 7, "one parse node");
  is(tree[0]+'', "test", "first text is correct");
  is(tree[2]+'', "spec!", "second text is correct")
  is(tree[4]+'', "spec!!", "third text is correct")
  is(tree[6]+'', "spec!!!", "last text is correct")
...
test("Basic Output As String", function() {
  ...
  is(textFunc, "function(context, partials){this.buffer.push('spec');};", "template renders correct text function.");
});

test("One Variable", function() {
  var text = "spec {{foo}} spec";
  var t = Hogan.compile(text);
  var s = t.render({foo:'bar'});
  is(s, "spec bar spec", "basic variable substitution works.");
});
...
test("One Variable As String", function() {
  ...
  is(funcText, "function(context, partials){this.buffer.push('spec ');\nthis.buffer.push(this.find('foo', context));\nthis.buffer.push(' spec');};",
...
test("Multiple Variables", function() {
  var text = "spec {{foo}} spec {{bar}} spec {{baz}} spec {{foo}} spec";
  var t = Hogan.compile(text);
  var s = t.render({foo:'42', bar: '43', baz: '44'});
  is(s, "spec 42 spec 43 spec 44 spec 42 spec", "all variables render correctly.");
});
